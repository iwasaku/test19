...
// 共有ボタン用
let postText = null;
const postURL = "https://iwasaku.github.io/test19/KRAGKS3/";
...
phina.define('Hema', {
    ...
    init: function (x, y, color) {
        ...
        this.setSize(HEMA_RADIUS * 2.1, HEMA_RADIUS * 2.1);
        ...
        this.rotOffset = Math.random() * 360;
    },
    ...
    updateFromBody: function () {
        if (this.body) {
            ...
            this.x = pos.x * 30;
            this.y = pos.y * 30;
            this.rotation = this.body.GetAngle() * (180 / Math.PI) + this.rotOffset;
        }
    },
...
phina.define("TitleScene", {
    ...
    init: function (option) {
        ...
        Label({
            ...
            align: "center",
            fill: 'white',
        }).addChildTo(this).setPosition(SCREEN_CENTER_X, SCREEN_CENTER_Y + SCREEN_HEIGHT * 1 / 4 + 6);
...
phina.define('MainScene', {
    ...
    init: function () {
        ...
        for (let ii = 0; ii < 6; ii++) {
            const offsets = [224, 0, 144, 192, 112, 0];
            bgSprites[ii] = Sprite("bg" + ii).addChildTo(group0).setPosition(SCREEN_CENTER_X, SCREEN_CENTER_Y + offsets[ii]);
    ...
    clearChain: function () {
        if (this.chain.length >= 3) {
            ...
            // コンボ表示更新
            if (this.combo > 1) {
                this.comboLabel.text = this.combo + ' コンボ!';
    ...
    enterParty: function () {
        ...
        // 背景BGをフェードイン
        let bgIndex = (this.partyCount - 1) % 6;
    ...
    explodeBomb: function (bomb) {
        ...
        // 爆発エフェクト
        //Explosion(bombX, bombY, HEMA_RADIUS * 10).addChildTo(group3);
        const explosion = Explosion(bombX, bombY, HEMA_RADIUS * 2).addChildTo(group3);
        explosion.tweener
            .to({ scaleX: 5.0, scaleY: 5.0 }, 600, 'linear')
            .call(() => {
                explosion.remove();
            });
        if (canVibrate) {
